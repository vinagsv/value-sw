...
import { Sun, Moon, FileText, Package, BarChart3, Settings, LogOut, Navigation } from 'lucide-react';
...
const AppContent = ({ theme, toggleTheme }) => {
  ...
  const navLinks = [
    ...
    { path: '/gatepasses', label: 'Gate Passes', icon: Navigation },
    { path: '/items', label: 'Inventory', icon: Package },
    { path: '/statistics', label: 'Dashboard', icon: BarChart3 },
    { path: '/settings', label: 'Settings', icon: Settings },
  ];

  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 print:min-h-0 print:block print:bg-white ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      <header className="h-16 px-6 mx-2 mt-2 flex justify-between items-center z-50 glass-card print:hidden">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black text-xl">
            V
          </div>
          <div>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link 
                  key={link.path} 
                  to={link.path} 
                  className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${isActive ? 'bg-white dark:bg-gray-700 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}`}
                >
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            title="Toggle Theme"
          >
            {theme === 'light' ? <Moon size={18} className="text-indigo-600" /> : <Sun size={18} className="text-yellow-400" />}
          </button>
          
          {user && (
            <button onClick={logout} className="p-2 rounded-xl bg-red-50 hover:bg-red-100 text-red-600 transition shadow-sm border border-red-200" title="Logout">
              <LogOut size={18} />
            </button>
          )}
        </div>
      ...
      <main className="flex-1 p-2 print:p-0">
        <Routes>
          ...
          <Route path="/settings" element={<ProtectedRoute><SettingsPage theme={theme}/></ProtectedRoute>} />
