...
import { Search, Plus, Package, Download, Check, Pencil, Trash2, Loader2, X, ChevronDown, Lock } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      ...
      >
        ...
        <div className="px-6 py-5">
          ...
          <p className={`text-sm font-semibold mb-3 ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>
            Save this edit to <span className={`font-bold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>{pending.field}</span>?
          </p>

          {/* Old → New value display */}
          <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl mb-5 text-sm
            ${isDark ? 'bg-gray-700/60 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
            <span className={`line-through truncate max-w-[130px] ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
              {String(pending.oldValue)}
            </span>
            <span className={`text-xs font-bold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>→</span>
            <span className={`font-semibold truncate max-w-[130px] ${isDark ? 'text-amber-300' : 'text-amber-700'}`}>
              {String(pending.newPayload[pending.field] ?? '')}
            </span>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={onConfirm}
              className="flex-1 py-2.5 rounded-xl text-sm font-bold bg-amber-500 hover:bg-amber-600 text-white transition shadow-sm shadow-amber-500/20"
            >
              Save Change
            </button>
          </div>
...
const EditableCell = memo(({ item, field, type = 'text', options = [], onSave, onRequestSave, isEditMode, isDark }) => {
  ...
  const handleCommit = (newValue) => {
    ...
    onRequestSave(
      ...
      async () => {
        setIsSaving(true);
        try { await onSave(item.id, { ...item, [field]: trimmed }); }
        catch { setValue(item[field] ?? ''); }
    ...
    return (
      <div className="relative min-w-[80px]" data-no-expand="true">
        <input list={listId} disabled={isSaving} value={value}
          ...
          className={base} />
        <datalist id={listId}>{options.map(o => <option key={o} value={o}>{o}</option>)}</datalist>
...
const ItemsPage = ({ theme }) => {
  ...
  const inp = (extra = '') =>
    ...
    ${isDark ? 'bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-500' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400'} ${extra}`;
  ...
  const tdBase = () => `p-0 ${isDark ? 'border-b border-gray-700/60' : 'border-b border-gray-100'}`;
  ...
  const cellProps = (item, field, extra = {}) => ({
    ...
    isEditMode: isEditMode && isAdmin, isDark, ...extra
  });

  return (
    <div className={`min-h-screen flex flex-col ${isDark ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
      <SaveConfirmPopup pending={pendingConfirm} onConfirm={handleConfirmSave} onCancel={handleCancelSave} isDark={isDark} />
      ...
      <div className="max-w-[1700px] w-full mx-auto p-4 md:p-6 flex flex-col gap-4" style={{ height: 'calc(100vh - 64px)' }}>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-lg font-bold uppercase tracking-wide ${isDark ? 'text-gray-100' : 'text-gray-800'}`}>Inventory</h2>
              <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                {items.length} items · {isAdmin ? 'click a row to expand and edit' : 'view-only mode'}
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {isAdmin ? (
              ...
              /* Non-admin: show locked indicator */
              <div className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold border
                ${isDark ? 'bg-gray-800/50 border-gray-700 text-gray-600' : 'bg-gray-100 border-gray-200 text-gray-400'}`}
                title="Only admins can edit inventory">
        ...
        {!isAdmin && (
          <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm
            ...
            <span>Inventory is in <strong>read-only mode</strong>. Contact an administrator to make changes.</span>
          </div>
        )}
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr>
                ...
                {isEditMode && isAdmin && <th className={th}>Actions</th>}
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr><td colSpan={isEditMode && isAdmin ? 9 : 8} className="text-center py-16 text-sm opacity-40">
                  <Loader2 className="w-5 h-5 animate-spin mx-auto mb-2" />Loading...
                </td></tr>
              ) : items.length === 0 ? (
                <tr><td colSpan={isEditMode && isAdmin ? 9 : 8} className="text-center py-16 text-sm opacity-40">No items found.</td></tr>
              ) : items.map(item => (
                <React.Fragment key={item.id}>
                  <tr
                    className={`group transition-colors cursor-pointer
                      ...
                      ${isEditMode && isAdmin ? (isDark ? 'border-l-2 border-l-amber-500/40' : 'border-l-2 border-l-amber-400/50') : ''}`}
                    onClick={e => handleRowClick(e, item.id)}>

                    <td className={`${tdBase()} pl-3 w-8`}>
                      <ChevronDown size={13} className={`transition-transform text-gray-400 ${expandedRow === item.id ? 'rotate-180' : ''}`} />
                    </td>
                    ...
                    <td className={tdBase()}><EditableCell {...cellProps(item, 'item_name')} /></td>
                    <td className={tdBase()}>
                      <div className={`font-semibold ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>
                        <EditableCell {...cellProps(item, 'available_quantity', {
                          type: 'number',
                          isEditMode: isEditMode && isAdmin && item.quantity_applicable?.toLowerCase() === 'yes'
                        })} />
                      </div>
                    </td>
                    <td className={tdBase()}><EditableCell {...cellProps(item, 'rate', { type: 'number' })} /></td>
                    <td className={tdBase()}><EditableCell {...cellProps(item, 'intra_state_tax_rate', { type: 'datalist', options: taxRateOptions })} /></td>
                    <td className={tdBase()}><EditableCell {...cellProps(item, 'inter_state_tax_rate', { type: 'datalist', options: taxRateOptions })} /></td>
                  ...
                  {expandedRow === item.id && (
                    <tr className={isDark ? 'bg-blue-900/10' : 'bg-blue-50/40'}>
                      <td colSpan={isEditMode && isAdmin ? 9 : 8} className={`px-6 py-4 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                          ...
                          <div className={`rounded-lg p-3 ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
                            ...
                            <div className="space-y-1.5">
                              <div className="flex justify-between items-center">
                                ...
                                <div className="text-xs font-medium"><EditableCell {...cellProps(item, 'hsn_sac')} /></div>
                              </div>
                              <div className="flex justify-between items-center">
                                <span className="text-xs text-gray-500">Product Type</span>
                                <div className="text-xs font-medium"><EditableCell {...cellProps(item, 'product_type', { type: 'select', options: productTypes })} /></div>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className={`rounded-lg p-3 ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
                            ...
                            <div className="space-y-1.5">
                              <div className="flex justify-between items-center">
                                ...
                                <div className="text-xs font-medium"><EditableCell {...cellProps(item, 'quantity_applicable', { type: 'select', options: qtyOptions })} /></div>
                              </div>
                              <div className="flex justify-between items-center">
                                <span className="text-xs text-gray-500">Taxable</span>
                                <div className="text-xs font-medium"><EditableCell {...cellProps(item, 'taxable', { type: 'select', options: ['true', 'false'] })} /></div>
                              </div>
                            </div>
                          </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="space-y-1.5">
                                <div className="flex justify-between items-center">
                                  ...
                                  <div className="text-xs font-medium"><EditableCell {...cellProps(item, 'intra_state_tax_name')} /></div>
                                </div>
                              </div>
                              <div className="space-y-1.5">
                                <div className="flex justify-between items-center">
                                  <span className="text-xs text-gray-500">Inter-State Name</span>
                                  <div className="text-xs font-medium"><EditableCell {...cellProps(item, 'inter_state_tax_name')} /></div>
                                </div>
                              </div>
                            </div>
                          </div>
                            ...
                            {isEditMode && isAdmin ? (
                              <textarea
                                ...
                                onBlur={e => {
                                  ...
                                  handleRequestSave(item.id, { ...item, description: newVal }, 'description', item.description || '',
                                    async () => handleCellSave(item.id, { ...item, description: newVal }),
                                    () => { e.target.value = item.description || ''; }
                                  );
                                }}
                                rows={2} data-no-expand="true"
                                className={`w-full text-sm rounded px-3 py-2 outline-none border resize-none transition-colors
                                  ${isDark ? 'bg-gray-700 border-gray-600 text-gray-200 focus:border-amber-500' : 'bg-amber-50/50 border-amber-200/60 text-gray-900 focus:border-amber-400'}`}
                              />
                            ) : (
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 custom-scrollbar">
              <div className="p-6 space-y-5">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Item ID" required>
                      <input required type="text" value={form.item_id} onChange={e => f('item_id', e.target.value)} className={inp('uppercase')} />
                    </Field>
                    <Field label="Item Name" required>
                      <input required type="text" value={form.item_name} onChange={e => f('item_name', e.target.value)} className={inp()} />
                    </Field>
                    <Field label="HSN / SAC Code">
                      <input type="text" value={form.hsn_sac} onChange={e => f('hsn_sac', e.target.value)} className={inp()} />
                    </Field>
                    <Field label="Status">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Rate (₹)" required>
                      <input required type="number" step="0.01" value={form.rate} onChange={e => f('rate', e.target.value)} className={inp()} />
                    </Field>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <button type="button" onClick={() => f('taxable', !form.taxable)}
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition ${form.taxable ? 'bg-blue-600' : isDark ? 'bg-gray-600' : 'bg-gray-300'}`}>
                      <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform ${form.taxable ? 'translate-x-[18px]' : 'translate-x-1'}`} />
                    </button>
                    ...
                    <div className="grid grid-cols-2 gap-5">
                      <div className={`rounded-lg p-4 border ${isDark ? 'bg-gray-700/50 border-gray-600' : 'bg-blue-50/50 border-blue-100'}`}>
                        ...
                        <div className="space-y-3">
                          <Field label="Tax Name"><input type="text" value={form.intra_state_tax_name} onChange={e => f('intra_state_tax_name', e.target.value)} className={inp()} /></Field>
                          <Field label="Tax Rate (%)">
                            <input list="intra-tax-rates" type="number" step="0.01" value={form.intra_state_tax_rate} onChange={e => f('intra_state_tax_rate', e.target.value ? Number(e.target.value) : '')} className={inp()} />
                            <datalist id="intra-tax-rates">{taxRateOptions.map(r => <option key={r} value={r} />)}</datalist>
                          </Field>
                        </div>
                      </div>
                      <div className={`rounded-lg p-4 border ${isDark ? 'bg-gray-700/50 border-gray-600' : 'bg-purple-50/50 border-purple-100'}`}>
                        ...
                        <div className="space-y-3">
                          <Field label="Tax Name"><input type="text" value={form.inter_state_tax_name} onChange={e => f('inter_state_tax_name', e.target.value)} className={inp()} /></Field>
                          <Field label="Tax Rate (%)">
                            <input list="inter-tax-rates" type="number" step="0.01" value={form.inter_state_tax_rate} onChange={e => f('inter_state_tax_rate', e.target.value ? Number(e.target.value) : '')} className={inp()} />
                            <datalist id="inter-tax-rates">{taxRateOptions.map(r => <option key={r} value={r} />)}</datalist>
                          </Field>
                        </div>
                      </div>
                    </div>
                ...
                <div>
                  ...
                  <textarea rows={3} value={form.description} onChange={e => f('description', e.target.value)} placeholder="Optional notes..." className={`${inp()} resize-none`} />
                </div>
              </div>
              <div className={`flex justify-end gap-3 px-6 py-4 border-t shrink-0 ${isDark ? 'border-gray-700' : 'border-gray-100'}`}>
                ...
                <button type="submit" className="px-6 py-2 rounded-lg text-sm font-bold bg-blue-600 hover:bg-blue-700 text-white transition">
                  Add to Inventory
                </button>
              </div>
            </form>
