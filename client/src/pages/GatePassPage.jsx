...
import { Search, Plus, Printer, Save, Trash2, Edit, Lock } from 'lucide-react';
...
const GatePassPage = ({ theme }) => {
  ...
  const handleEdit = (gp) => {
    setForm({
      id: gp.id, gate_pass_no: gp.gate_pass_no,
  ...
  const handleSave = async (e) => {
    ...
    try {
      let savedData;
      if (form.id) savedData = await updateGatePass(form.id, form);
      else savedData = await createGatePass(form);
      setCurrentGP(savedData.gatePass);
      loadGatePasses(searchTerm);
      setViewMode('preview');
    } catch { alert('Failed to save Gate Pass'); }
  };

  const handleDelete = async (id) => {
    if (!isAdmin) {
      alert('Only administrators can delete gate passes.');
      return;
    }
    ...
    try {
      ...
      loadGatePasses(searchTerm);
      setViewMode('list');
    } catch { alert('Failed to delete Gate Pass'); }
  };
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 11.5, fontFamily: 'sans-serif' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end' }}>
          ...
          <span style={{ borderBottom: '1px dotted black', flexGrow: 1, padding: '0 8px', fontWeight: 700, textTransform: 'uppercase', fontSize: 12 }}>{gp.customer_name || ''}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', flex: 1 }}>
            <span style={{ fontWeight: 700, marginRight: 8, textTransform: 'uppercase', flexShrink: 0 }}>Mobile No:</span>
            <span style={{ borderBottom: '1px dotted black', flexGrow: 1, padding: '0 8px', fontWeight: 700, textTransform: 'uppercase' }}>{gp.customer_mobile || ''}</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end', flex: 1 }}>
            <span style={{ fontWeight: 700, marginRight: 8, textTransform: 'uppercase', flexShrink: 0 }}>Veh Reg No:</span>
            <span style={{ borderBottom: '1px dotted black', flexGrow: 1, padding: '0 8px', fontWeight: 700, textTransform: 'uppercase' }}>{gp.vehicle_reg_no || ''}</span>
          </div>
        </div>
        {gp.ref_bill_no && (
          <div style={{ display: 'flex', alignItems: 'flex-end' }}>
            <span style={{ fontWeight: 700, marginRight: 8, textTransform: 'uppercase', flexShrink: 0, width: 120 }}>Ref Inv No:</span>
            <span style={{ borderBottom: '1px dotted black', flexGrow: 1, padding: '0 8px', fontWeight: 700, textTransform: 'uppercase', fontSize: 12, color: '#dc2626' }}>{gp.ref_bill_no}</span>
          </div>
        )}
      </div>
  ...
  const GatePassForPrint = ({ gp }) => (
    <div style={{ width: '210mm', background: '#fff', color: '#111', fontFamily: "'Georgia', 'Times New Roman', serif", fontSize: 12, lineHeight: 1.55, boxSizing: 'border-box', padding: '10mm 14mm 8mm' }}>
  ...
  return (
    <div className={`flex print:block print:h-auto ${isDark ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`} style={{ height: 'calc(100vh - 64px)', overflow: 'hidden' }}>

      {viewMode === 'preview' && currentGP && printPortalEl &&
        createPortal(<GatePassForPrint gp={currentGP} />, printPortalEl)}

      {/* Sidebar */}
      <div className={`flex-shrink-0 flex flex-col border-r print:hidden ${isDark ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`} style={{ width: 300 }}>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          {isLoading ? (
            ...
            <div key={gp.id} onClick={() => { setCurrentGP(gp); setViewMode('preview'); }}
      ...
      {/* Main area */}
      <div className={`flex-1 print:hidden ${isDark ? 'bg-gray-900' : 'bg-gray-100'}`} style={{ overflow: 'auto', minWidth: 0 }}>
        <div style={{ padding: 24 }}>

          {/* FORM */}
          {viewMode === 'form' && (
            <div className={`p-6 rounded-xl border shadow-sm ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`} style={{ maxWidth: 672, margin: '0 auto' }}>
          ...
          {viewMode === 'preview' && currentGP && (
            <div style={{ width: '210mm' }}>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginBottom: 16 }}>
                <button onClick={() => handleEdit(currentGP)}
                ...
                {/* Delete — admin only */}
                {isAdmin ? (
                  <button onClick={() => handleDelete(currentGP.id)}
                  ...
                  <div className={`px-4 py-2 text-sm font-bold rounded-lg flex items-center gap-2 cursor-not-allowed opacity-50
                    ${isDark ? 'bg-gray-700 text-gray-500' : 'bg-gray-100 text-gray-400'}`}
                    title="Only admins can delete gate passes">
                    <Lock size={16} /> Delete
                  </div>
                )}
              ...
              <div style={{ width: '210mm', background: '#fff', color: '#111', fontFamily: "'Georgia', 'Times New Roman', serif", fontSize: 12, lineHeight: 1.55, boxSizing: 'border-box', padding: '10mm 14mm 8mm', borderRadius: 12, boxShadow: '0 2px 16px rgba(0,0,0,0.10)', border: '1px solid #e5e7eb' }}>
