...
import { BarChart3, TrendingUp, IndianRupee, Layers, Download, Calendar, X, Trash2, ClipboardList, Search, ChevronLeft, ChevronRight, RefreshCcw } from 'lucide-react';
  ...
  return (
    <div className={`flex items-center justify-between gap-1 pt-3 mt-3 border-t ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
      <button
        ...
        disabled={!canPrev}
        className={`p-1.5 rounded-lg border transition flex items-center
          ${canPrev
...
const StatisticsPage = ({ theme }) => {
  ...
  // Load audit logs on first render
  useEffect(() => {
    loadAuditLogs(1, '');
  }, [loadAuditLogs]);
  ...
  const executeExport = async () => {
    ...
    try {
      ...
      if (exportFrom && exportTo) {
        dataToExport = await fetchBills({ fromDate: exportFrom, toDate: exportTo });
      } else {
        dataToExport = await fetchBills({});
      }
      ...
      const excelData = dataToExport.map(b => ({
        ...
        'GSTIN': b.customer_gstin || 'N/A',
        'Vehicle Reg No': b.vehicle_reg_no || 'N/A',
        'Taxable Subtotal (₹)': Number(b.subtotal),
        'Total Tax (₹)': Number(b.total_tax),
      ...
      const dateSuffix = exportFrom && exportTo
  ...
  const StatCard = ({ title, value, subtitle, icon: Icon, color }) => {
    ...
    return (
      <div className={`${card} p-6 relative overflow-hidden group shadow-sm`}>
        <div className={`absolute -right-6 -top-6 opacity-10 group-hover:scale-110 transition-transform duration-500 ${colorClasses[color] || 'text-gray-400'}`}>
          <Icon size={120} />
        </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className={`flex border p-1 rounded-lg backdrop-blur-md ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              {['day', 'week', 'month'].map(p => (
                <button
                  key={p}
                  onClick={() => setPeriod(p)}
                  className={`px-4 py-1.5 text-sm font-bold uppercase rounded-md transition-all ${
                    period === p
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold border transition
                ${isDark ? 'bg-red-900/30 border-red-700/50 text-red-400 hover:bg-red-900/50' : 'bg-red-50 border-red-200 text-red-600 hover:bg-red-100'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold border transition
                ${showExportPanel
                  ? isDark ? 'bg-green-700 border-green-600 text-white' : 'bg-green-600 border-green-700 text-white'
        ...
        {showExportPanel && (
          <div className={`p-4 rounded-xl border shadow-sm animate-fadeIn ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-center mb-3">
              <h3 className={`text-sm font-bold flex items-center gap-2 ${isDark ? 'text-green-400' : 'text-green-700'}`}>
                <Calendar size={15} /> Export Invoices to Excel
              </h3>
              <button
                onClick={() => setShowExportPanel(false)}
                className={`p-1 rounded transition ${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-gray-700'}`}>
                <X size={16} />
              </button>
            </div>
            <p className={`text-xs mb-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              Select a date range to export specific bills, or leave blank to export all bills.
            </p>
        ...
        {isLoading || !stats ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <StatCard
                title="Total Revenue"
                value={`₹${Number(stats.summary?.total_revenue || 0).toLocaleString('en-IN')}`}
                subtitle="Active bills only"
                icon={IndianRupee}
                color="green"
              />
              <StatCard
                title="Bills Generated"
                value={stats.summary?.total_bills || 0}
              ...
              <div className={`${card} p-6 shadow-sm`}>
                <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3 flex items-center gap-2">
                  <Layers size={14} /> External Bills Overview
                </h3>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`pl-7 pr-3 py-1.5 rounded-lg border text-xs outline-none focus:ring-2 focus:ring-blue-500 transition w-44
                    ${isDark ? 'bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-500' : 'bg-gray-50 border-gray-200 text-gray-800'}`}
                />
              </div>
              {/* Refresh */}
              <button
                onClick={() => loadAuditLogs(auditPage, auditSearch)}
                ...
                <tbody>
                  {auditLogs.map((log) => {
                    ...
                    return (
                      <tr
                        key={log.id}
                        className={`border-b transition-colors ${isDark ? 'border-gray-700/60 hover:bg-gray-700/30' : 'border-gray-100 hover:bg-gray-50/60'}`}
                      >
