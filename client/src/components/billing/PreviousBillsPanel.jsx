...
import { Search, Ban, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
import { fetchBills } from '../../api/bills';

const PAGE_SIZE = 15;

// FIX #1: Accept isAdmin and onDeleteBill props so admin can delete individual bills
const PreviousBillsPanel = ({ onSelectBill, theme, refreshTrigger, isAdmin = false, onDeleteBill }) => {
  ...
  return (
    <div className="flex flex-col h-full px-3 py-3 relative">
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            className={`w-full pl-8 pr-3 py-1.5 text-xs rounded-lg border focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition
              ${isDark ? 'bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-500' : 'bg-gray-50 border-gray-200 text-gray-800 placeholder-gray-400'}`}
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-1.5 -mx-1 px-1 custom-scrollbar">
        {loading ? (
          <div className={`text-center text-xs mt-10 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>Loading bills...</div>
        ) : bills.length === 0 ? (
          <div className={`text-center text-xs mt-10 ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>No bills found.</div>
        ) : (
          paginated.map(bill => (
            <div
              key={bill.id}
              className={`px-3 py-2.5 rounded-lg border transition-all relative group
                ${isCancelled(bill)
                  ? isDark ? 'bg-red-900/20 border-red-800/60 hover:bg-red-900/30' : 'bg-red-50 border-red-100 hover:bg-red-100/60'
                  : isDark ? 'bg-gray-700/50 border-gray-600/80 hover:bg-gray-700 hover:border-gray-500' : 'bg-white border-gray-100 hover:bg-blue-50/40 hover:border-blue-100'
                }`}
            >
              {/* Clickable content area */}
              <div className="cursor-pointer" onClick={() => onSelectBill(bill)}>
                <div className="flex justify-between items-center mb-1">
                  <span className={`font-bold text-xs tabular-nums
                    ${isCancelled(bill) ? isDark ? 'text-red-400' : 'text-red-500' : isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                    {bill.bill_no}
                  </span>
                ...
                <div className={`text-xs font-medium truncate mb-1.5 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  {bill.customer_name || '—'}
                </div>

                <div className="flex justify-between items-center">
                  <span className={`text-xs font-bold tabular-nums
                    ${isCancelled(bill) ? isDark ? 'text-red-400' : 'text-red-500' : isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
              ...
              {/* FIX #1: Admin delete button — appears on hover, positioned top-right */}
              {isAdmin && onDeleteBill && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onDeleteBill(bill);
                  ...
                  className={`absolute top-2 right-2 p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity
                    ${isDark
                      ? 'bg-red-900/50 text-red-400 hover:bg-red-800/70 hover:text-red-300'
      ...
      {!loading && totalPages > 1 && (
        <div className={`flex-shrink-0 mt-2 pt-2 border-t flex items-center justify-between gap-1
          ...
          <button onClick={() => goTo(safePage - 1)} disabled={!canPrev}
            className={`p-1.5 rounded-lg border transition flex items-center justify-center
              ${canPrev
                ? isDark ? 'border-gray-600 text-gray-300 hover:bg-gray-700 hover:border-gray-500' : 'border-gray-200 text-gray-600 hover:bg-gray-100'
                  ...
                  <button onClick={() => goTo(p)}
                    className={`min-w-[26px] h-[26px] rounded-md text-xs font-bold border transition
                      ${p === safePage
                        ...
                        : isDark ? 'border-gray-600 text-gray-400 hover:bg-gray-700 hover:text-gray-200' : 'border-gray-200 text-gray-500 hover:bg-gray-100 hover:text-gray-700'}`}>
                    {p}
                  </button>
                </React.Fragment>
              );
          ...
          <button onClick={() => goTo(safePage + 1)} disabled={!canNext}
            className={`p-1.5 rounded-lg border transition flex items-center justify-center
              ${canNext
                ? isDark ? 'border-gray-600 text-gray-300 hover:bg-gray-700 hover:border-gray-500' : 'border-gray-200 text-gray-600 hover:bg-gray-100'
                : isDark ? 'border-gray-700 text-gray-600 cursor-not-allowed' : 'border-gray-100 text-gray-300 cursor-not-allowed'}`}>
